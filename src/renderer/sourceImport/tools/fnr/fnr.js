        ...
        if (toWhat == "toString") {
            ...
            if (itemFound.length == 1 || itemFound[0].toUpperCase() == itemFound[1].toUpperCase()) {
                ...
                for (let k = 0; k < itemFound.length; ++k) {
                    resultStringArray[ind++] = toUpperCase ? removeNoise(String(itemFound[k])).toUpperCase() : removeNoise(String(itemFound[k]));
                    if (printToJSON) JSON_cleaned_obj[JSON_item_name] = resultStringArray;
